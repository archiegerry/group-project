...
"""
GNews API scraper
Download GNews articles using specified search terms and upload to S3.

USAGE:
python gnews_download.py           FOR SCRAPING THEN UPLOADING
python gnews_download.py scrape    FOR SCRAPING ONLY
python gnews_download.py upload    FOR UPLOADING ONLY
...
def compress_and_upload():
    print("Compressing and uploading GNews articles...")
...
def scrape():
    print("Scraping GNews articles...")
    # Create working directory
    working_dir = Path("working/gnews")
    working_dir.mkdir(exist_ok=True, parents=True)

    # Load base 
    stocks = pd.read_csv(Path("../Stocks/data/stock_list.csv"))
...
# Main function to determine whether to scrape or upload or both
def main():
